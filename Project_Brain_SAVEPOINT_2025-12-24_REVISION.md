...
### Revision smoke test (known-good)
```powershell
# Create a base order
$body = @{ artist="SP Test"; asset_type="radio"; notes="base for revise smoke" } | ConvertTo-Json
$base = irm "http://127.0.0.1:8000/orders/new" -Method Post -ContentType "application/json" -Body $body

# Revise it
irm "http://127.0.0.1:8000/orders/$($base.id)/revise" -Method Post | ConvertTo-Json -Depth 8
```
...
### 7) Revision endpoint (implemented)
- Added `POST /orders/{order_id}/revise`
- Creates a new Order as a revision of an existing Order:
  - Copies `artist`, `asset_type`, `notes` (and optional workflow fields)
  - Sets `is_revision=true`, `parent_order_id` to the parent order
  - Generates a new SP for radio/video and sets:
    - `new_order.sp.revision_of = parent.sp.sp_number`
- Example chain verified:
...
def revise_order(order_id: int, db: Session = Depends(get_db)):
    ...
    payload = OrderCreate(
        ...
        # workflow fields (optional in model)
    ...
    # Link new SP back to prior SP number (radio/video only)
    if getattr(parent, "sp", None) and getattr(new_order, "sp", None):
        new_order.sp.revision_of = parent.sp.sp_number

        # Mirror if Order has a revision_of column too
...
- Standalone SP endpoints (optional, but useful):
  - `GET /sp/{sp_id}` (or `/sp_master/{id}`) to fetch a single SP record
  - `GET /sp/` to list SP records (later: filters by order_type, date, etc.)
Notes:
- As of this save point, **there are no `/sp` routes** — SP is only returned nested on Orders via `OrderResponse.sp`.
